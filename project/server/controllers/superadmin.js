...
exports.shippingData = async (req, res) => {
  ...
  res.json({
    shippingRate: superadmin.shippingRate,
    shippingCost: superadmin.shippingCost,
  });
};
...
exports.editBanner = async (req, res) => {
  ...
  if (req.file) {
    ...
    // fs.unlinkSync(Path);
    let files = [
      `public/uploads/${banner.bannerPhoto}`,
...
exports.editDispatcher = async (req, res) => {
  let dispatcher = await Dispatcher.findById(req.body.dispatcher_id).select(
    "-password -salt -resetPasswordLink"
  );
  if (!dispatcher) {
  ...
  if (req.body.oldPassword && req.body.newPassword) {
    let disPatcher = await Dispatcher.findByCredentials(
      dispatcher.email,
...
exports.blockUnbolckDispatcher = async (req, res) => {
  let dispatcher = await Dispatcher.findById(req.params.dispatcher_id).select(
    "-password -salt -resetPasswordLink"
  );
  if (!dispatcher) {
    return res.status(404).json({ error: "Dispatcher not found." });
...
exports.flipAdminWarehouseApproval = async (req, res) => {
  ...
  if (!warehouse) {
    return res
      .status(404)
      .json({ error: "No warehouse information available" });
  }
...
exports.flipAdminAccountApproval = async (req, res) => {
  ...
  if (adminAccount.emailVerifyLink) {
    return res
      .status(403)
      .json({ error: "Admin email has not been verified." });
  }
  ...
  if (adminAccount.emailVerifyLink) {
    return res
      .status(403)
      .json({ error: "Admin's email has not been verified." });
  }
  if (!adminAccount.businessInfo.isVerified) {
    return res
      .status(403)
      .json({ error: "Admin's business information has not been verified." });
  }
  if (!adminAccount.adminBank.isVerified) {
    return res
      .status(403)
      .json({ error: "Admin's bank information has not been verified." });
  }
  if (!adminAccount.adminWareHouse.isVerified) {
    return res
      .status(403)
      .json({ error: "Admin's warehouse information has not been verified." });
  }
...
exports.blockUnblockAdmin = async (req, res) => {
  let admin = await Admin.findById(req.params.id).select(
    "-password -salt -resetPasswordLink -emailVerifyLink"
  );
  if (!admin) {
...
exports.approveProduct = async (req, res) => {
  ...
  if (!categories.length) {
    return res
      .status(404)
      .json({ error: "Categories not found of this product." });
  }
...
exports.disApproveProduct = async (req, res) => {
  ...
  const updateProduct = product.toObject();
  (updateProduct.isVerified = null), (updateProduct.isRejected = Date.now());
